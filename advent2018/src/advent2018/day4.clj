...
(def id-res (reduce maintain-max (into [] sleep)))

(def minu (first (reduce (fn [r1 r2] (if (< (last r1) (last r2)) r2 r1)) (into [] (frequencies (map #(t/minute (tc/from-long %)) (last id-res)))))))

(println (* (read-string (name (first id-res))) minu))
